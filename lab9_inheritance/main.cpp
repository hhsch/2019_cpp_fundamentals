...
void TestOne()
{
	...
	shapes.push_back(new Ellipse(7.7, 8.8));
	shapes.push_back(new Trapezoid);
	shapes.push_back(new Trapezoid(9.9, 10.10, 11.11));
	shapes.push_back(new Sector);
	shapes.push_back(new Sector(12.12, 13.13));
...
void TestTwo()
{
	...
	shapes.push_back(new RectangularPyramid(4.4, 5.5, 6.6));
...
void RandomShapes(vector<Shape *> &shapes)
{
	...
	{
		int shapeType = RandomInt(0, 10);
		Shape *shape = nullptr;
		if (shapeType == 0) shape = new Square(rf);
		if (shapeType == 1) shape = new Rectangle(rf, rf);
		if (shapeType == 2) shape = new Triangle(rf, rf);
		if (shapeType == 3) shape = new Circle(rf);
		if (shapeType == 4) shape = new Ellipse(rf, rf);
		if (shapeType == 5) shape = new Trapezoid(rf, rf, rf);
		if (shapeType == 6) shape = new Sector(rf, rf);
		if (shapeType == 7) shape = new TriangularPyramid(rf, rf, rf);
		if (shapeType == 8) shape = new RectangularPyramid(rf, rf, rf);
		if (shapeType == 9) shape = new Cylinder(rf, rf);
		if (shapeType == 10) shape = new Sphere(rf);
...
void Random3DShapes(vector<Shape3D *> &shapes)
{
	...
	for (int i = 0; i < shapeCount; i++)
	{
		...
		if (shapeType == 1) shape = new RectangularPyramid(rf, rf, rf);
		if (shapeType == 2) shape = new Cylinder(rf, rf);
		if (shapeType == 3) shape = new Sphere(rf);
		shapes.push_back(shape);
	}
}
